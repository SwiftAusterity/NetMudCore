...
namespace NetMudCore.DataAccess.FileSystem
{
    public class PlayerData : FileAccessor
    {
        ...
        public IPlayer RestorePlayer(string accountHandle, IPlayerTemplate chr)
        {
            ...
            try
            {
                ...
                if (Directory.Exists(playerDirectory + "Inventory/"))
                {
                    ...
                    {
                        IInanimate blankObject = Activator.CreateInstance("NetMudCore.Data", "NetMudCore.Data.Game.Inanimate") as IInanimate;
        ...
        public bool LoadAllCharactersForAccountToCache(string accountHandle)
        {
            ...
            foreach (FileInfo file in charDirectory.EnumerateFiles("*.playertemplate", SearchOption.AllDirectories))
            {
                try
                {
                    ...
                    System.Runtime.Remoting.ObjectHandle blankEntity = Activator.CreateInstance("NetMudCore.Data", "NetMudCore.Data.Players.PlayerTemplate");
        ...
        private void WritePlayer(DirectoryInfo dir, IPlayer entity)
        {
            ...
            try
            {
                ...
                foreach (IInanimate obj in entity.Inventory.EntitiesContained())
                {
                    ...
                    LiveData.WriteSpecificEntity(entityDirectory, obj);
                }
            }
            catch (Exception ex)
            {
        ...
        private static string GetPlayerFilename(IPlayer entity)
        {
            return GetPlayerFilename(entity.TemplateId);
        }

        private static string GetPlayerFilename(long charId)
        {
        ...
        /// <param name="entity">The player in question</param>
        /// <returns>the filename</returns>
        private static string GetCharacterFilename(IPlayerTemplate entity)
        {
