...
namespace NetMudCore.Data.Architectural.EntityBase
{
    ...
    public class DimensionalModelData : TemplatePartial, IDimensionalModelData
    {
        ...
        public bool IsModelValid()
        {
            ...
            {
                //2d has 21 planes, but they're all flat (21 X nodes)
                DimensionalModelType.Flat => ModelPlanes.Count == 21 && !ModelPlanes.Any(plane => string.IsNullOrWhiteSpace(plane.TagName) || plane.ModelNodes.Count != 21),
                //0d is always valid, it doesn't care about the model
                DimensionalModelType.None => true,
                _ => false,
            };
        }
            ...
            try
            {
                ...
                foreach (string myString in delimitedPlanes.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries))
                {
                    ...
                    foreach (string nodeString in currentLineNodes.Skip(1))
                    {
                        ...
                        if (nodeStringComponents.Length > 1 && string.IsNullOrWhiteSpace(nodeStringComponents[1]))
                        {
