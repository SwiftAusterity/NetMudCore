...
namespace NetMudCore.Interp
{
    ...
    {
        private static IEnumerable<ILexeme> _currentDictionary => ConfigDataCache.GetAll<ILexeme>();
                    ...
                    foreach ((KeyValuePair<string, IDictata> value, int i) innerVerb in innerBrands.Select((value, i) => (value, i)))
                    {
                        ...
                        if (index > 0 && index < innerBrands.Count - 1)
                        {
                            ...
                            if(wordBefore.WordType == LexicalType.Conjunction)
                            {
                                ...
                                int bumpCount = sentence[..(splitIndex - 1)].Count(ch => ch == ',' || ch == ':');

                                string sentenceOne = sentence[..(splitIndex - 1 + bumpCount)];
                                string sentenceTwo = sentence[(splitIndex + wordBefore.Name.Length + bumpCount)..];
        ...
        private IList<Tuple<string, bool>> IsolateIndividuals(string baseString)
        {
            ...
            foreach (string returnString in originalStrings)
            {
                ...
                {
                    int i = DataUtility.TryConvert<int>(returnString[2..^2]);
         ...
         */
        private static IList<Tuple<string, bool>> ParseCommaListsOut(ref int iterator, ref string baseString)
        {
        ...
        private static IList<Tuple<string, bool>> RunListPattern(Regex capturePattern, ref int iterator, ref string baseString)
        {
            List<Tuple<string, bool>> foundStrings = new();
        ...
        private static List<Tuple<string, bool>> ParseQuotesOut(ref string baseString, ref int iterator)
        {
            List<Tuple<string, bool>> foundStrings = new();
        ...
        private static string IsolateStrings(string baseString, string closure, List<Tuple<string, bool>> foundStrings, ref int foundStringIterator)
        {
        ...
        static IEnumerable<string> IsolateSentences(string input)
        {
            List<string> sentences = new();
        ...
        private static IList<string> RemoveGrammaticalNiceities(IList<string> currentParams)
        {
