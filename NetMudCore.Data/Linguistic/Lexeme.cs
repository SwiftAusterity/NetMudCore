...
namespace NetMudCore.Data.Linguistic
{
    public class Lexeme : ConfigData, ILexeme, IComparable<ILexeme>, IEquatable<ILexeme>, IEqualityComparer<ILexeme>
    {
        ...
        public Lexeme()
        {
            WordForms = Array.Empty<IDictata>();
            Name = string.Empty;
            IsSynMapped = false;
        ...
        public void FillLanguages()
        {
            ...
            foreach (ILanguage language in otherLanguages)
            {
                ...
                {
                    LexicalContext context = new(null, new Gender() { Feminine = word.Feminine }, language)
                    {
                        Language = language,
                ...
                if(newLexeme.WordForms.Length > 0)
                {
        ...
        public IDictata GetForm(LexicalType form, string[] semantics, bool bestFit = true)
        {
            ...
            {
                var expectedMinimumSemanticSimilarity = semantics.Length / 2;
