...
namespace NetMudCore.Combat
{
    public static class Round
    {
        ...
        public static bool ExecuteRound(IMobile actor, IMobile target)
        {
            ...
            if (attack == null || !actor.Executing)
            {
                ...
                if (myCombo == null)
                {
                    ...
                    attack = attacks.Where(atk => atk.IsValid(actor, target, (ulong)Math.Abs(actor.Balance), null)).OrderBy(atk => Guid.NewGuid()).FirstOrDefault();
                }
                else
                {
        ...
        private static Dictionary<FightingArtComboUsage, double> GetUsageWeights(IMobile actor, IMobile target)
        {
