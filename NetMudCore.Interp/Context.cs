...
namespace NetMudCore.Interp
{
    ...
    public class Context : IContext
    {
        ...
        public Context(string fullCommand, IActor actor)
        {
            ...
            if (potentialCommands == null || !potentialCommands.Any())
            {
            ...
            foreach (CommandPackage currentCommand in potentialCommands)
            {
                ...
                try
                {
                    ...
                    if (currentCommand.InputRemainder.Any())
                    {
        ...
        public void SeekInDirection(CommandPackage command, Type paramType, CommandParameterAttribute currentNeededParm)
        {
            ...
            while (parmDirWords > 0)
            {
                ...
                {
                    disambiguator = int.Parse(currentParmString[..currentParmString.IndexOf(".")]);
                    currentParmString = currentParmString[(currentParmString.IndexOf(".") + 1)..];
                }
        ...
        public void SeekInBackingData<T>(CommandPackage command, CommandParameterAttribute currentNeededParm) where T : ITemplate
        {
            ...
            while (parmWords > 0)
            {
                ...
                T validObject;
                if (!long.TryParse(currentParmString, out long parmID))
                {
        ...
        private static IList<string> RemoveGrammaticalNiceities(IEnumerable<string> currentParams)
        {
